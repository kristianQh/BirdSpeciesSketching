...
def parse_arguments():
    ...
    parser.add_argument("--genus", type=str, default="none",
                        help="guiding text for abstracting the sketch based on CLIP score")

    # =================================
    # ============ wandb ==============
    # =================================
    parser.add_argument("--use_wandb", type=int, default=0)
    parser.add_argument("--wandb_user", type=str, default="kristianqh")
    ...
    parser.add_argument("--attention_init", type=int, default=1,
                        help="if True, use the attention heads of CLIP model to set the location of the initial strokes")
    parser.add_argument("--kp_init", type=int, default=0,
                        help="if True, use keypoints from CUB to set the location of the initial strokes. (Image HAS to be loaded from CUB in run_object_sketching.py)")
    parser.add_argument("--attention_text", type=str, default="head beak,tail,legs",  # !currently only works with 3 text/attention heads!
                        help="initialize location of initial strokes based on CLIP attention")
    parser.add_argument("--attention_dist", type=str, default="5,4,3",
                        help="number of points used for each attention text for initializing stroke locations")
    ...
    parser.add_argument("--stage2_color", type=int, default=1,
                        help="If set True, stage 2 will color only the strokes using L2 loss")
    parser.add_argument("--color_iters", type=int, default=400,
                        help="Numbers of iterations to run in stage 2 of L2 coloring")
    ...
    parser.add_argument("--percep_loss", type=str, default="none",
                        help="the type of perceptual loss to be used (L2/LPIPS/model_conv_loss/none)")
    parser.add_argument("--perceptual_weight", type=float, default=0,
                        help="weight the perceptual loss")
    parser.add_argument("--clip_score", type=int, default=0),
    ...
    parser.add_argument("--optim_width", type=float, default=0,
                        help="")
    parser.add_argument("--optim_color", type=float, default=0,
                        help="")
    parser.add_argument("--model_name", type=str, default="inception_v3",
                        help="name of the timm model to be used for convolutional/perceptual loss")
    ...
    args.attention_text = [
        str(item) for item in args.attention_text.split(',')]

    args.attention_dist = [
        int(item) for item in args.attention_dist.split(',')]
