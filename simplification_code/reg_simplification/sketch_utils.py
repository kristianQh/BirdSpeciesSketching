...
def inference_sketch(args, eps=1e-4):
    ...
    num_control_points = torch.zeros(
        1, dtype=torch.int32) + (control_points_per_seg - 2)
    ...
    if 0:
        mlp_width = WidthMLP(num_strokes=num_paths,
                             num_cp=control_points_per_seg).to(device)
        checkpoint = torch.load(mlp_width_weights_path)
        mlp_width.load_state_dict(checkpoint['model_state_dict'])

    points_vars, canvas_width, canvas_height = get_init_points(
        sketch_init_path)
    ...
    if 0:  # first iter use just the location mlp
    ...
    for p in range(num_paths):
        ...
        w = width / 1.5
        path = pydiffvg.Path(
            num_control_points=num_control_points, points=all_points[:, p].reshape(
                (-1, 2)),
        ...
        if is_in_canvas_ and w > 0.7:
            ...
            shape_groups.append(path_group)
    pydiffvg.save_svg(f"{output_path}/best_iter.svg",
                      canvas_width, canvas_height, shapes, shape_groups)
