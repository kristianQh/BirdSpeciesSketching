...
class Loss(nn.Module):
    def __init__(self, args):
        ...
        if self.percep_loss != "none":
            if self.percep_loss == "l2":
                self.loss_mapper["l2"] = L2_()
            elif self.percep_loss == "lpips":
                self.loss_mapper["lpips"] = LPIPS(device=args.device).to(args.device)
            elif self.percep_loss == "moel_conv_loss":
                self.loss_mapper["model_conv_loss"] = ModelConvLoss(args)
    ...
    def get_losses_to_apply(self):
        ...
        if self.clip_score:
            losses_to_apply.append("clip_score")
    ...
    def forward(self, sketches, targets, color_parameters, renderer, epoch, points_optim=None, mode="train"):
        ...
        for loss_name in self.losses_to_apply:
            if loss_name == "clip_score":
                losses_dict[loss_name] = self.loss_mapper[loss_name](
                    sketches, targets, mode)
            if loss_name == "clip_conv_loss" or loss_name == "model_conv_loss":
...
class CLIPScoreLoss(nn.Module):
    def __init__(self, args, label_emb, hg_clip_model):
        super(CLIPScoreLoss, self).__init__()
        self.args = args
        self.device = args.device

        
        self.model = hg_clip_model
        self.label_emb = label_emb

    def forward(self, sketch, target, mode="train"):
        x = sketch.to(self.device)
        pred = classify_images(x, self.model, self.label_emb, self.device)
        return -(pred*0.05)
...
class CLIPConvLoss(torch.nn.Module):
    ...
    def forward(self, sketch, target, mode="train"):
        ...
        if self.clip_model_name.startswith("RN"):
            ...
            xs_conv_features = self.visual_encoder(xs)
            ys_conv_features = self.visual_encoder(ys)
